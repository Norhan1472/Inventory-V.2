...
public class ServerServiceImpl implements ServerService {
    ...
    @Override
    public Collection<Server> getAllServersUp(int limit) {
        log.info("Fetching all Servers up");
        List<Server>servers =  serverRepo.findAll(PageRequest.of(0,limit)).toList();
        var result = new ArrayList<Server>();
        for(int i=0;i< servers.size();i++){
            if(servers.get(i).getStatusServer().name().equals("SERVER_UP")){
    ...
    @Override
    public Collection<Server> getAllServersDown(int limit) {
        log.info("Fetching all Servers down");
        List<Server>servers =  serverRepo.findAll(PageRequest.of(0,limit)).toList();
        var result = new ArrayList<Server>();
        for(int i=0;i< servers.size();i++){
            if(servers.get(i).getStatusServer().name().equals("SERVER_DOWN")){
                result.add(servers.get(i));
            }
        }
        return result;
